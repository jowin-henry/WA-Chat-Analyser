...
def getemojistats(selected_user, df):
    ...
    emojis = []
    for message in df['Message']:
        # Use emoji.is_emoji to filter emojis properly
        emojis.extend([c for c in message if emoji.is_emoji(c)])

    emojidf = pd.DataFrame(Counter(emojis).most_common())
    return emojidf
...
def monthtimeline(selected_user, df):
    ...
    time = [f"{temp['Month'][i]}-{temp['Year'][i]}" for i in range(temp.shape[0])]
